...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        BOOST_FOREACH(const CMutableTransaction& txv, txVariants) {
            if (txv.vin.size() > i) {
                sigdata = CombineSignatures(prevPubKey, MutableTransactionNoWithdrawsSignatureChecker(&txConst, i, amount), sigdata, DataFromTransaction(txv, i));
            }
        }
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, MutableTransactionNoWithdrawSignatureChecker(&txConst, i, amount), &serror)) {
