...
void CWalletTx::SetBlindingData(unsigned int nOut, CAmount amountIn, CPubKey pubkeyIn, uint256 blindingfactorIn, const CAsset& assetIn, uint256 assetBlindingFactorIn) const
{
    ...
    if (mapValue["blindingdata"].size() < (nOut + 1) * 138) {
        mapValue["blindingdata"].resize(tx->vout.size() * 138);
    }
            ...
            while (true)
            {
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nFeeRet > nFeeNeeded && nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                        ...
                        change_position->nValue = CTxOutValue(change_position->nValue.GetAmount() + extraFeePaid);
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    if (change_position->nValue.GetAmount() >= MIN_FINAL_CHANGE + additionalFeeNeeded) {
                        change_position->nValue = CTxOutValue(change_position->nValue.GetAmount() - additionalFeeNeeded);
...
{
    if (output.nValue.IsAmount() && output.nAsset.IsExplicit()) {
