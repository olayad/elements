...
UniValue sendtomainchain_pak(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            "sendtomainchain address amount ( subtractfeefromamount ) \n"
            ...
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
            "1. \"address\"        (string, required) Dummy argument. Must be \"\". Only for non-PAK `sendtomainchain` compatibility.\n"
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "initpegoutwallet",                 &initpegoutwallet,              {"bitcoin_xpub", "bip32_counter_counter", "liquid_pak"} },
