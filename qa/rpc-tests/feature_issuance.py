...
def process_raw_issuance(node, issuance_list):
    ...
    issued_tx = node.rawissueasset(funded_tx, issuance_list)
    ...
    for issuance_req, tx_input in zip(issuance_list, decoded_tx["vin"]):
...
class IssuanceTest (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        issued_tx = self.nodes[2].rawissueasset(funded_tx, [{"asset_amount":1, "asset_address":nonblind_addr}])
        ...
        # to match since blindrawtransaction does all issuances or none. Don't feel like a more
        # verbose API really helps in any realstic scenario.
